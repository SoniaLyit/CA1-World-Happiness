...
# Sort the happiness Score 
sorted_happiness_score <- world_happiness1[order(world_happiness1$Happiness_Score),]
...
# Plot a correlation graph for the data set with pairs()
...
#H0-GDP rate is not affectable by positive affect
#H1- GDP rate is increasing with the increase in positive affect 
...
# Statically Test
plot(world_happiness1$Positive_effect, world_happiness1$GDP , 
     xlab = "Positive_effect", ylab = "GDP" )
#Statical Test: We will apply the Spearman's  or Kendall's test here.
cor.test(world_happiness1$Positive_effect, 
         world_happiness1$GDP, method = 'kendall')
...
#H0- Happiness_Score of a country is not affected by the Social_Support
#H1- Social_Support Play a crucial role to increase the  Happiness_Score of a country
...
cor.test(world_happiness1$Social_Support, world_happiness1$Happiness_Score, 
         method = 'pearson')
...
#---------Hypothesis -3 ------
# H0 - score between India and Afghanistan are the same in all the years
# H1 - Happiness in India more than Afghanistan in all the years
...
shapiro.test(data_com$Happiness_Score)
# data is normal

t.test(Happiness_Score ~ Country)
...
#H0-(GDP) play  role in determining the happiness of a country
